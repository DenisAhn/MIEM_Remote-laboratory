...
function createData(number, date, time, compile, video, details) {
            ...
            alignItems: 'center',
                ...
                <TableContainer component={Paper} sx={{ mt: 4, ml: 2 }}>
                    <Table sx={{ minWidth: 650 }} aria-label="caption table">
                        ...
                        <TableBody>
                            {rows.map((row) => (<TableRow key={row.time}>
                                    <TableCell component="th" scope="row">{row.number}</TableCell>
                                    <TableCell align='center'>{row.date}</TableCell>
                                    <TableCell align="center">{row.time}</TableCell>
                                    <TableCell align="center">
                                        <Link href={`URL_TO_LOG_FILE/${row.compile}`} download={row.compile}>
                                            Лог комиляции
                                        </Link>
                                    </TableCell>
                                    <TableCell align="center">
                                        <Link href={`URL_TO_VIDEO_FILE/${row.video}`} download={row.video}>
                                            Видео
                                        </Link>
                                    </TableCell>
                                    <TableCell align="center">
                                        <Button sx={{ textTransform: 'none' }} onClick={handleGoToDemo}>
                                            {row.details}
                                        </Button></TableCell>
                                </TableRow>))}
                        </TableBody>
