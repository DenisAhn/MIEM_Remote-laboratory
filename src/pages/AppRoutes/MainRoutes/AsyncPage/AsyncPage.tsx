...
const AsyncPage = () => {
    ...
    const [buttonClicks, setButtonClicks] = useState<Buttons[]>([]);
    const [textFile, setTextFile] = useState<FileProps>(null);
    const [sofFile, setSofFile] = useState<FileProps>(null);
    ...
    const handleIncrement = (id: number) => {
        const updatedButtonClicks = buttonClicks.map((button) =>
            button.id === id ? { ...button, counter: button.counter + 5 } : button
        );
    ...
    const handleDecrement = (id: number) => {
        const updatedButtonClicks = buttonClicks.map((button) =>
            button.id === id && button.counter > 0 ? { ...button, counter: button.counter - 5 } : button
        );
        setButtonClicks(updatedButtonClicks);
    };
    ...
    const handleResetCounter = (id: number) => {
        const updatedButtonClicks = buttonClicks.map((button) =>
            button.id === id ? { ...button, counter: 0 } : button
        );
        setButtonClicks(updatedButtonClicks);
    };
    ...
    const handleCombinedSubmit = async () => {
        if (isFormValid) {
            try {
                ...
                formData.append('textFile', textFile as File);
                formData.append('sofFile', sofFile as File);
                        ...
                        >
                            <Stack>
                                <Typography
                                    variant={'h6'}
                                    sx={{ ml: 4, mb: 1, mt: 2 }}
                                >Список нажатий и задержек:</Typography>
                                <Divider/>
                                <Button
                                    variant="contained"
                                    color="primary"
                                        ...
                                        >
                                            ...
                                            <Box>
                                                ...
                                                <IconButton
                                                    size="small"
                                                    color="error"
                                                    onClick={() => handleDeleteRow(button.id)}
                                                >
                                                    <DeleteIcon />
                                                </IconButton>
                                            </Box>
